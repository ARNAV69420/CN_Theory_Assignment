...
{
    string s;
    connector server(PORT,SOCK_STREAM);
    server.clearstructures();
    server.fillserver();
    server.bindsocket();
    server.listenForClients();
    while(1){
        int filedes=server.acceptNow();
        fd.pb(filedes);
        clients.pb(fd.size()-1);
        pthread_t t;
        pthread_create(&t,NULL,reader,&clients[clients.size()-1]);
    }
    server.closeServer();
    return 0;
}	
