...
{
    //lets read the directory   
    vector<array<string,2>> contacts;
    map<string,string> numberof;
    map<string,string> nameof;
    ifstream file("directory.csv");
    string line;
    getline(file, line);//get the field names
    while (getline(file, line)) {
        stringstream ss(line);
        string name, phone_number;
        getline(ss, name, ',');
        getline(ss, phone_number);
        nameof[phone_number]=name;
        numberof[name]=phone_number;
    }
    ...
    while(1)
    {
        ...
        if(isalpha(s[0])){
            if(numberof.count(s))
                ...
                server.sendy("Not Found");
        }
        else{
            if(nameof.count(s))
                server.sendy(nameof[s]);
            else
                server.sendy("Not Found");
        }
    }
